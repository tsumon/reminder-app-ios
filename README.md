...
四个底部 Tab：首页 / 日历 / 统计 / 设置。AI 在设置里配置，不是独立 Tab。

首页标题是「提醒事项」：工具栏搜索、筛选只有「全部 / 今天 / 本周」，列表分「提醒中 / 等待中」。到期行内「确认」；重试中副文案「还没确认 · HH:MM 再响」（HH:MM 是下次再响时刻）。日历点选日期后列出当天任务。统计三项：本月完成 / 连续天数 / 完成率（空状态 0%），另有打卡城堡、本周花园、最常忘记时段。
...
│   ├── StatsView.swift            # 统计洞察（本月完成/连续天数/完成率）
